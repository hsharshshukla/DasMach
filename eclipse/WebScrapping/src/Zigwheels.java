...
public static void main(String args[]) throws Exception{
	String website = "https://www.cartrade.com";	//Base Url 
	//Setting up File for Model Specific Details
...
//Get list of All Models for a Brand	  
	   d = Jsoup.connect(url).timeout(6000*6000).get();
	   ...
	   for (Element element : ele.select("div.content")){
		   ...
		   url = website + href;
//Get list of All Variants of that Model		   
		   vd = Jsoup.connect(url).timeout(6000*6000).get();
		   ...
		   for (Element element_vd : ele_vd.select("a[class=brdlinkred]")){
			   ...
			   url = website + href_vd;
//Get info of that Variant			   
			   System.out.println(var_name);
			   vd_var = Jsoup.connect(url).timeout(6000*6000).get();
			   ...
			   for (Element element_vd_var_col : ele_vd_var_col.select("div[style]")){
				   ...
				   var_col = var_col.substring(var_col.indexOf("#")+1,var_col.length()) ;
				   System.out.println(var_col);
			   }
		   }
